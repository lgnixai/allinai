...
const Setup = () => {
  ...
  const sendVerificationCode = async () => {
    ...
    try {
      const res = await API.get(`/api/phone_verification?phone=${phone}&purpose=register`);
      const { success, message } = res.data;
      if (success) {
        showNotice(t('验证码已发送，请查收'));
      } else {
  ...
  const onSubmit = () => {
    ...
    // For root_init=false, validate admin username and phone
                  ...
                  <Card className="!rounded-2xl shadow-sm border-0 mb-6">
                    <div className="flex items-center mb-4 p-6 rounded-xl" style={{
                      ...
                      <div className="relative">
                        ...
                        <div style={{ color: '#ffffff' }} className="text-sm opacity-80">{t('设置系统管理员的手机号登录信息')}</div>
                      </div>
                    </div>
                    ...
                    {setupStatus.root_init ? (
                      ...
                      <>
                        ...
                        <Form.Input
                          ...
                          validateStatus="default"
                          onChange={(value) =>
                            setFormData({ ...formData, phone_verification_code: value })
                          }
                        />
                      </>
                    )}
