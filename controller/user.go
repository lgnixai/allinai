...
func Login(c *gin.Context) {
	...
	verificationCode := loginRequest.PhoneVerificationCode
...
func Register(c *gin.Context) {
	...
	if !common.VerifyCodeWithKey(user.Phone, user.PhoneVerificationCode, common.PhoneVerificationPurpose) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "手机验证码错误或已过期",
		})
	...
	exist, err := model.CheckUserExistOrDeleted(user.Phone)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "数据库错误，请稍后重试",
	...
	// 生成用户名（基于手机号）
	username := "user_" + user.Phone[len(user.Phone)-4:] // 使用手机号后4位作为用户名后缀
	...
	// 清理验证码字段，不保存到数据库
	cleanUser.VerificationCode = ""
	cleanUser.PhoneVerificationCode = ""
	...
	if err := model.DB.Where("phone = ?", cleanUser.Phone).First(&insertedUser).Error; err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
	...
	if constant.GenerateDefaultToken {
		...
		token := model.Token{
			...
			Name:               cleanUser.DisplayName + "的初始令牌",
			Key:                key,
...
func ResetPasswordWithNewPassword(c *gin.Context) {
	var req struct {
		Phone    string `json:"phone"`
		Token    string `json:"token"`
	...
	err := json.NewDecoder(c.Request.Body).Decode(&req)
	if req.Phone == "" || req.Token == "" || req.Password == "" {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "无效的参数",
		})
	...
	err = model.ResetUserPasswordByPhone(req.Phone, req.Password)
	if err != nil {
		common.ApiError(c, err)
		return
	}
