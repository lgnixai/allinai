...
func CheckUserExistOrDeleted(phone string) (bool, error) {
	var user User

	var err error
	err = DB.Unscoped().First(&user, "phone = ?", phone).Error
	if err != nil {
...
// ValidateAndFill check phone verification code & user status
func (user *User) ValidateAndFill() (err error) {
	// When querying with struct, GORM will only query with non-zero fields,
	// that means if your field's value is 0, '', false or other zero values,
	// it won't be used to build query conditions
	phone := strings.TrimSpace(user.Phone)
	verificationCode := strings.TrimSpace(user.PhoneVerificationCode)
	if phone == "" || verificationCode == "" {
		return errors.New("手机号或验证码为空")
	}
	// find by phone
	...
	// 验证手机验证码
	if !common.VerifyCodeWithKey(phone, verificationCode, common.PhoneVerificationPurpose) {
