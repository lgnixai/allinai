...
import { IconGithubLogo, IconMail, IconLock, IconPhone, IconKey } from '@douyinfe/semi-icons';
...
const LoginForm = () => {
  ...
  const { phone, phone_verification_code } = inputs;
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (phone && phone_verification_code) {
  ...
  const sendVerificationCode = async () => {
    ...
    try {
      const res = await API.get(
        `/api/phone_verification?phone=${inputs.phone}&purpose=login&turnstile=${turnstileToken}`,
      );
  ...
  return (
    <div className="relative overflow-hidden bg-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-sm mt-[64px]">
        {showPhoneLogin || !(status.github_oauth || status.oidc_enabled || status.wechat_login || status.linuxdo_oauth || status.telegram_oauth)
