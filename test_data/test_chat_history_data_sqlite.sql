...
SELECT 
    '时间分布检查' as test_name,
    datetime(MIN(created_time), 'unixepoch') as earliest_message,
    datetime(MAX(created_time), 'unixepoch') as latest_message,
    COUNT(*) as total_messages,
    ROUND((MAX(created_time) - MIN(created_time)) / 3600.0, 2) as hours_span
FROM chat_histories 
WHERE user_id IN (1, 2, 3);
...
SELECT 
    ...
    SUM(tokens) as total_tokens,
    ROUND(SUM(cost), 6) as total_cost,
    datetime(MIN(created_time), 'unixepoch') as start_time,
    datetime(MAX(created_time), 'unixepoch') as end_time
FROM chat_histories 
WHERE user_id IN (1, 2, 3)
GROUP BY session_id, user_id
ORDER BY message_count DESC
